...
DebuggerModel.prototype = {
  ...
  setBreakpoint: function(file, lineNumber) {
  ...
  startDebug(device) {
    this.stopDebug();
    this.startRuntimeInChrome(device);
    this._inspectorModel.stopInspector();
  },

  stopDebug: function() {
    console.log("Debugger action stopDebug");
    this._deviceModel.stopDebug(this.device ? this.device.deviceId : null);  // Stop debug mode
    this._killDebugger();
    this._killRuntime();
    this._inspectorModel.startInspectorService(this.device);
  },
  ...
  startDebuggerService: function() {
    ...
    var This = this;
    var queryUrl = `http://${ThePkgCfgs.debugger.host}:${ThePkgCfgs.debugger.port}/json`;
