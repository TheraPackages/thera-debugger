...
DebuggerModel.prototype = {
  ...
  stopDebug: function() {
    ...
    setTimeout(startInspector.bind(this), 666);
    function startInspector() {
      console.log('Start inspector service when quit debug.');
      this._inspectorModel.startInspectorService(this.getTargetDevice());  // This is a default action
    }
  },
  ...
  _innerStopDebug: function() {
    var device = this.getTargetDevice();
    if (device) {
      this._deviceModel.stopDebug(device.deviceId);
    }
    this._killDebugger();
    this._killRuntime();
  },
  ...
  startRuntimeInChrome: function(device) {
    ...
    var host = PkgCfgs.server.host, port = PkgCfgs.server.port;
    var runtimeUrl = `http://${host}:${port}/debugger.html?sessionId=${device.debuggerSessionId}#new`;
    console.log("Debugger runtimeUrl: " + runtimeUrl);

    // Setup runtime environment properties.
    var runtime = this._runtime = document.createElement('webview');
    runtime.src = this._runtimeUrl = runtimeUrl;
    ...
    runtime.addEventListener('dom-ready', (stdEvent) => {         //3
      runtime.domReady = true;
      // console.log('Debugger process dom-ready', stdEvent);
    });
    runtime.addEventListener('did-finish-load', (stdEvent) => {   //4
      console.log('Runtime did-finish-load', stdEvent);
    });
    runtime.addEventListener('did-stop-loading', (stdEvent) => {
      // This event will enter twice. Donnot put unrepeatable things here.
      if (!runtime.didStopLoadingFlag) {
        console.log('Runtime did-stop-loading', stdEvent);
        runtime.didStopLoadingFlag = true;
        self.startDebuggerService();  // Continue to start debugger process.
      }
    });
  ...
  startDebuggerService: function() {
    ...
    var This = this;
    var queryUrl = `http://localhost:${PkgCfgs.debugger.port}/json`;
    ...
    function handlePageList(pageList) {
      ...
      var targetPage = null;
      for (idx in pageList) {
        if (This._runtimeUrl.startsWith(pageList[idx].url)) {
      ...
      if (targetPage) {
        // Format target url.
        var toolPage = PkgCfgs.project.root + '/devtools_ex/inspector.html';
  ...
  reloadRuntime: function() {
    if (this._runtime && this._runtime.domReady) {
      this._runtime.reload();
    }
  },

  reloadDebugger: function() {
    if (this._debugger && this._debugger.domReady) {
