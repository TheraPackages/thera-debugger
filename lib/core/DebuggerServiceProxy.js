...
class DebuggerServiceProxy extends DebugService {
  ...
  // Array contains breakpoints
  syncAllBreakpoints (breakpoints) {
    if (!breakpoints || !breakpoints.length) {
      console.log('syncAllBreakpoints push empty array.');
      return;
    }
    // Clone array.
    breakpoints = JSON.parse(JSON.stringify(breakpoints));
    breakpoints.forEach((breakpoint) => {
      breakpoint.line = this.uiDataAdapter.offsetLineNumber(breakpoint.line);
    });
    this.debuggerModel.syncAllBreakpoints(breakpoints);
  }
