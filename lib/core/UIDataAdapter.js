...
var StartedPayload = Payload.StartedPayload;
var StoppedPayload = Payload.StoppedPayload;
var RemoteObject = Scope.RemoteObject;
var PropertyDescriptor = Scope.PropertyDescriptor;
const IncSourceRegExp = /(^http)/;
const ExcSourceRegExp = /(\?)/;
...
class UIDataAdapter {
  ...
  parse(eventName, args) {
    switch(eventName) {
      ...
      case DebugEvent.DebuggerStarted:  // 自定义
        return this.debuggerStarted();
      case DebugEvent.DebuggerStopped:  // 自定义
  ...
  */
  ...
  uiSourceCodeAdded(sourceCode) {
    ...
    if (IncSourceRegExp.test(sourceURL) && !ExcSourceRegExp.test(sourceURL)) {
      var localURL = null;
      var expSourceLib = /(http:\/\/localhost:\d+\/((source)|(lib))\/)/g;
      if (expSourceLib.test(sourceURL)) {       // source/libs/**.js
        localURL = sourceURL.substr(sourceURL.match(expSourceLib)[0].length);
        console.log('Accepted source file: ' + sourceURL);
      } else {
        console.log('Unmatched source file: ' + sourceURL);
      }
      // Print debuggable source to console-panel.
      atom.commands.dispatch(atom.views.getView(atom.workspace), 'console:log', ['Source: ' + sourceURL, 'log']);
      ...
      // Get file content.
      NetUtils.curl(sourceURL, (function curlHandler(err, response) {
        if (err) {
      ...
      // console.warn('Reject: ' + sourceURL, IncSourceRegExp.test(sourceURL) , ExcSourceRegExp.test(sourceURL));
    }
  }
