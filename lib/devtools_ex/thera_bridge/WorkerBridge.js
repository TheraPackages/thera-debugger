...
WebInspector.WorkerBridge = function() {
  ...
  this._incSourceRegExp = /(^http)/;
  this._excSourceRegExp = /(\?)/;
...
WebInspector.WorkerBridge.prototype = {
  ...
  _setBreakpoint: function(breakpoint) {
    ...
    if (!uiSourceCode) {
      ...
      if (breakpoint.enabled) {
        var retBrk = WebInspector.breakpointManager.setBreakpoint(uiSourceCode,
          breakpoint.lineNumber, breakpoint.columnNumber,
          breakpoint.condition || '', breakpoint.enabled);
          this._removeBreakpointFromStorage(retBrk);
      } else {
  ...
  /**
   * Do not keep breakpoint in storage.
   * @param {!WebInspector.BreakpointManager.Breakpoint}
   */
  _removeBreakpointFromStorage: function(breakpoint) {
    if (breakpoint) {
      WebInspector.breakpointManager._storage._removeBreakpoint(breakpoint);
    }
  },
  ...
  _handleUISourceCodeAdded: function(event) {
    ...
    if (this._incSourceRegExp.test(sourceURL)) {  // exec/test/match
  ...
  _handleParsedScriptSource: function(event) {
    ...
    if (this._incSourceRegExp.test(sourceURL)) {  // exec/test/match
      var scriptSource = this.sourceUrlInChrome[sourceURL] || {};
      scriptSource.sourceURL = sourceURL;
      scriptSource.localURL = undefined; // parse
