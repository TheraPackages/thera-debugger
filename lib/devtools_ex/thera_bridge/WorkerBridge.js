...
WebInspector.WorkerBridge.prototype = {
  ...
  _evaluateOnSelectedCallFrame: function(evalId, params) {
    ...
    // @param {function(?WebInspector.RemoteObject, !RuntimeAgent.ExceptionDetails=)}
    function evalCallback(remoteObject, hasException, rRemoteObject, exceptionDetails) {
      var object = this._dataConverter.remoteObject(remoteObject);
      this._invokeHostIpc(WebInspector.WorkerBridge.Channels.ChannelSend, "Debugger.EvaluateOnSelectedCallFrame", evalId, object);
  ...
  _evaluate: function(evalId, params) {
    ...
    function evalCallback(remoteObject, hasException, rRemoteObject, exceptionDetails) {
      var object = this._dataConverter.remoteObject(remoteObject);
      this._invokeHostIpc(WebInspector.WorkerBridge.Channels.ChannelSend, "Runtime.Evaluate", evalId, object);
      console.log(object, arguments);
    }
    execCtxs[0].evaluate(
      params.expression,        // expression: "floatVar\n\n"
