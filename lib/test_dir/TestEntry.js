...
TestEntry.prototype = {
  ...
  initTestPanel(packageThis) {
    ...
    addElement(panel, "AdbPush", "#ff00ff", () => {
      this.notifyPreviewServerAddress("30.7.78.66:7001");
    })
  ...
  testAndroidEmulator: function() {
    ...
    this.listAndroidEmulators().then((emulators) => {
      ...
      console.log(res);
      return _this.installApp('~/Desktop/falcon-release.apk');
    }).catch((error) => {
      console.error(error);

    }).then((res) => {
      console.log(res);
      return _this.launchApp('com.alibaba.falcon/com.alibaba.falcon.activity.MainActivity');
    }).catch((error) => {
      console.error(error);

    }).then((res) => {
      console.log(res);
      return this.notifyPreviewServerAddress(`${ip.address()}:7001`);
    }).catch((error) => {
      console.error(error);
      ...
      function startIfNoLiveEmulator(resolve, reject) {
        childProcess.exec(`adb devices -l`, (error, stdout, stderr) => {
          if (error) {
            ...
            if (numEmu) {
              resolve(`Host is running ${numEmu} emulators. Use them instead of launch a new on.`);
            } else {
      ...
      childProcess.exec(`adb -e install -rtdg ${apkName}`,
        (error, stdout, stderr) => {
          if (error) {
            ...
            resolve('Install android apk successfully.')
          }
        }
      )
    });
  ...
  notifyPreviewServerAddress: function(hostPort) {
    return new Promise(function(resolve, reject) {
